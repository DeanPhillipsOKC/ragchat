...
class SqLiteCollectionRepository(ICollectionRepository, SqLiteRepository):
    ...
    def add(self, collection):
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('INSERT INTO collections (id, data) VALUES (?, ?)', self._entity_to_row(collection))
            conn.commit()
    ...
    def list(self) -> list[Collection]:
        with sqlite3.connect(self.db_path) as conn:
            ...
            return [self._row_to_entity(row, Collection) for row in cursor.fetchall()]
    ...
    def select(self, guid: UUID):
        with sqlite3.connect(self.db_path) as conn:
            ...
            row = cursor.fetchone()
            if row:
                selected_id = "selected"  # Use a known, fixed ID for the single selected collection
                # Upsert into selected_collection
                cursor.execute('INSERT OR REPLACE INTO selected_collection (id, collection_id) VALUES (?, ?)',
    ...
    def get_selected(self) -> Collection:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT collections.* FROM collections JOIN selected_collection ON collections.id = selected_collection.collection_id')
            row = cursor.fetchone()
            if row:
                return self._row_to_entity(row, Collection)
            return None
