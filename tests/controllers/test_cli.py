...
def setup(mocker):
    ...
    mock_documents_controller = mocker.MagicMock()
    mocker.patch(
        "ragchat.controllers.cli.CollectionsController",
        return_value=mock_collections_controller,
    )

    # Initialize the Cli with the mocked CollectionsController
    cli = Cli(
        collections_controller=mock_collections_controller,
        utilities_controller=mock_utilities_controller,
        documents_controller=mock_documents_controller,
    )
