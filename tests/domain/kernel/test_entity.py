...
def test_entity_hash():
    ...
    assert hash(entity1) == hash(
        entity2
    ), "Entities with the same ID should have the same hash"
    assert hash(entity1) != hash(
        entity3
    ), "Entities with different IDs should not have the same hash."
...
def test_entity_inequality_with_different_types():
    ...
    assert (
        entity != non_entity
    ), "Entity should not be considered equal to an object of a different type"
